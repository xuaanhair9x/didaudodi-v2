...
class NewsLabelTb extends AbstractTableGateway
{
    ...
    {
        $select = new Select();
        $select ->from(array('nl' => $this->table));
        ...
        /*
            $select->join(
                ...
                new Expression('nl.id = l.item_id AND l.display = 1 AND l.name <> "" AND l.type = "'.$this->table.'"'.((empty($params['alllang']) ? ' AND l.language = "'.LANG.'"' : ''))),
        ...
        */
        ...
        $select->where->equalTo('nl.display', 1);
        $select->order(new Expression('CASE WHEN nl.sort IS NULL THEN 1 ELSE 0 END, nl.sort ASC, nl.id ASC'));
    ...
    public function getItem($params = null)
    {
        ...
        /*
            $select->join(
                array('l' => 'lang_multi_tb'),
                new Expression('nl.id = l.item_id AND l.type = "'.$this->table.'" AND l.language = "'.LANG.'"'),
                array('translate','language')
            );
        */

        $select->where->equalTo('nl.id', $params['id']);
        $select->where->equalTo('nl.display', 1);
