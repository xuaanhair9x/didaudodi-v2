...
class SectionTb extends AbstractTableGateway
{
    ...
    public function listItem($params = null)
    {
        ...
        /*
            $select->join(
                ...
                new Expression('s.id = l.item_id AND l.display = 1 AND l.type = "'.$this->table.'"'.((empty($params['alllang']) ? ' AND l.language = "'.LANG.'"' : ''))),
                array('translate','language')
            );
        */

        if (isset($params['root_id'])) {
            $select->where->equalTo('s.menu_code_id', $params['root_id']);
        }
        ...
        if (isset($params['special'])) {
            $select->where->like('s.special', '%'.$params['special'].'%');
        }

        $select->where->equalTo('s.display', 1);
        $select->order(new Expression('CASE WHEN s.sort IS NULL THEN 1 ELSE 0 END, '.(($params['orderby']) ? 's.'.implode(', s.', $params['orderby']) : 's.sort ASC, s.id ASC')));
    ...
    public function getItem($params = null)
    {
        ...
        /*
            $select->join(
                array('l' => 'lang_multi_tb'),
                new Expression('s.id = l.item_id AND l.type = "'.$this->table.'" AND l.language = "'.LANG.'"'),
                array('translate','language')
            );
        */
        ...
        $select->where->equalTo('s.id', $params['id']);
        $select->where->equalTo('s.display', 1);
