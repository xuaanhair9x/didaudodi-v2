...
CKEDITOR.plugins.add('placeholder_select',
{
	...
	init : function( editor )
	{
		...
		for (var i = 0; i < config.placeholders.length; i++) {
            ...
            config.placeholders[i].html = config.placeholders[i].html ? config.placeholders[i].html : config.placeholders[i].title;
			placeholders.push([placeholder, config.placeholders[i].html, config.placeholders[i].title]);
		}

		// add the menu to the editor
		editor.ui.addRichCombo('placeholder_select',
		{
			label: 		editor.lang.panelVoiceLabel,
			title: 		editor.lang.panelVoiceLabel,
			...
			init: function()
			{
				this.startGroup( editor.lang.panelVoiceLabel );
