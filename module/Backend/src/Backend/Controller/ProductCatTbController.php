...
class ProductCatTbController extends AbstractActionController
{
    ...
    public function onDispatch(\Zend\Mvc\MvcEvent $e)
    {
        ...
        $e->getRouteMatch()->setParam('active', 'product_tb-'.$this->_params['root_id']);
        ...
        if ($id) {
            $this->_cate = $this->_ProductCatTb->getItem(array('id' => $id));
            $this->_field = $this->_cate['define_item'];
    ...
    public function listAction()
    {
        ...
        if ($this->_field['level'] > 1) {
            $data['filter'] = $this->_ProductCatTb->listItem(array('root_id' => $this->_params['root_id'],'level' => '< 2'));
        ...
        $data['list'] = $this->_renderer->cateSortNested(
            $this->_ProductCatTb->listItem(array(
                'root_id' => $data['query']['parent'] ? $data['query']['parent'] : $this->_params['root_id'],
                'count' => true,
                'multiChoice' => ($this->_cate['define_product']['cat_id'] == 'checkbox') ? true : false,
    ...
    public function fieldAction()
    {
        ...
        if ($this->_params['id']) {
            ...
            $data['item'] = $this->_ProductCatTb->getItem(array('id' => $this->_params['id']));
            // Dữ liệu translate
            if ($this->identity()->lang) {
                $data['item']['translate'] = $this->_LangMultiTbController->listItem($this->_params);
            }
            // Danh sách id thương hiệu và phân loại của danh mục cấp cha
            if ($data['item']['parent'] != $this->_params['root_id']) {
                $list_id = $this->_ProductCatTb->getItem(array('id' => $data['item']['parent']));
                $list_brand_id = array('list_id' => explode(',', str_replace(':','',$list_id['list_brand_id'])));
                $list_label_id = array('list_id' => explode(',', str_replace(':','',$list_id['list_label_id'])));
            } else {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($validate->isError() == true) {
                ...
                if ($this->_params['checkForm']['image']) {
                    $this->_Upload->uploadImage($this->_params['post'], $this->_params['checkForm']['image'], array_merge($this->_field, array('listLang' => $this->identity()->langlist, 'lang' => $this->identity()->lang)), $data['item'], 'pcat'.$this->_params['id']);
                }
                // Xử lý file
                ...
                if ($this->_field['level']) {
                    $level = $this->_ProductCatTb->getItem(array('id' => $this->_params['post']['parent'], 'columns' => array('id','parent','level')));
                ...
                if (isset($this->_params['post']['list_label_id']) && $this->_params['post']['level'] == 1) {
                    ...
                    foreach ($this->_params['post']['list_label_id']  as $id) {
                        $ProductLabelTb->saveData(array('id' => $id, 'post' => array('status' => 1)));
                    }
                    ...
                    if ($this->_params['id'] && $data['item']['list_label_id']) {
                        ...
                        if ($labelIDs) {
                            $list_cat = $this->_ProductCatTb->listItem(array('root_id' => $this->_params['id'], 'list_label_id' => $labelIDs));
                            foreach ($list_cat as $value) {
                                $value['list_label_id'] = array_diff(explode(',', str_replace(':','',$value['list_label_id'])), $labelIDs);
                                $this->_ProductCatTb->saveData(array('id' => $value['id'],'post' => array('list_label_id' => $value['list_label_id'])));
                            }
                            foreach ($labelIDs as $id) {
                                if (!$this->_ProductCatTb->listItem(array('root_id' => $this->_params['root_id'], 'deny_id' => array($this->_params['id']), 'list_label_id' => array($id)))) {
                                    $ProductLabelTb->saveData(array('id' => $id, 'post' => array('status' => 0)));
                                }
                            }
                        }
                    }
                ...
                // Lưu dữ liệu
                $id = $this->_ProductCatTb->saveData($this->_params, $this->_field);

                // Thêm/sửa menu_public_tb
                $menuParent = $this->_MenuPublicTb->getItem(array('active' => 'product-list-'.$this->_params['root_id'], 'link' => '-'.$this->_params['post']['parent'].'-'.$this->_field['action']));
                $menu = ($this->_params['id']) ? $this->_MenuPublicTb->getItem(array('active' => 'product-list-'.$this->_params['root_id'], 'link' => '-'.$id.'-'.$this->_field['action'])) : '';
                $arrs = array();
        ...
        if ($this->_field['list_brand_id']) {
            $ProductBrandTb = new ProductBrandTb();
            $fields['list_brand_id'] = array(
                'type' => 'checkbox',
                'input' => true,
                'list' => $ProductBrandTb->listItem(array_merge(array('display' => 1), $list_brand_id)),
                'value' => explode(',', str_replace(':','',$data['item']['list_brand_id']))
            );
            $fields['parent']['attr'][] = '_HTHelper.loadHtml('.htmlspecialchars(json_encode(array(
                'url' => $this->url()->fromRoute('admincp',array('controller' => $this->_params['__CONTROLLER__'],'action' => 'brand','root_id' => $this->_params['root_id'])),
                'position' => 'list_brand_id',
        ...
        if ($this->_field['list_label_id']) {
            ...
            $fields['list_label_id'] = array(
                ...
                'list' => $ProductLabelTb->listItem(array_merge(array('display' => 1, 'display_lang' => $this->identity()->lang), $list_label_id))
            );
            $fields['parent']['attr'][] = '_HTHelper.loadHtml('.htmlspecialchars(json_encode(array(
        ...
        if ($this->_field['list_select_id']) {
            $this->_field['list_select_id'] = json_decode($this->_field['list_select_id'], true);

            $MenuCodeTb = new MenuCodeTb();
            $select = $MenuCodeTb->getItem(array('id' => $this->_field['list_select_id']['id'], 'columns' => array('id','name','define')));

            $ProductSelectTb = new ProductSelectTb();
            $fields['list_select_id'] = array(
                'type' => $select['define']['level'] == 2 ? 'group' : 'checkbox',
                'label' => $this->_field['list_select_id']['name'],
                'value' => explode(',', str_replace(':','',$data['item']['list_select_id'])),
            ...
            $fields['list_product_id'] = array(
                ...
                'cate' => array(
                    ...
                    'attr' => 'onchange="_HTHelper.loadHtml('.htmlspecialchars(json_encode(array(
                                'url' => $this->url()->fromRoute('admincp',array('controller' => $this->_params['__CONTROLLER__'],'action' => 'relatedProduct','id' => $this->_params['id'],'root_id' => $this->_params['root_id'])),
        ...
        if ($this->_field['list_news_id']) {
            ...
            foreach ($this->_field['list_news_id'] as $id) {
                ...
                $fields['list_news_id']['cate']['list'] = array_merge(
                    ...
                    ($item['level'] > 0 ? $this->_renderer->cateSortNested($NewsCatTb->listItem(array('root_id' => $id,'columns' => array('id','name','parent','level'))), array('parent' => $id, 'type' => 'sort')) : array(array_replace($item, array('level' => 1))))
                );
            }
    ...
    public function changeAction()
    {
        if ($this->getRequest()->isPost()) {
            ...
            foreach (explode(',',$this->_params['id']) as $id) {
                $this->_ProductCatTb->saveData(array('id' => $id, 'post' => $post));
                if (isset($post['sort']) || isset($post['special'])) {
                    // sort || display menu_public_tb liên quan
                    $menu = $this->_MenuPublicTb->getItem(array('active' => 'product-list-'.$this->_params['root_id'],'link' => '-'.$id.'-'.$this->_field['action']));
                    if ($menu) {
    ...
    {
        if ($this->getRequest()->isPost()) {
            echo $this->_ProductCatTb->listItem($this->params()->fromPost()) ? true : false;
        }
        return $this->getResponse();
    }
    ...
    {
        $data = array();
        $item = $this->_ProductCatTb->getItem(array('id' => $this->params()->fromPost('parent')));
        $arrs['display'] = 1;
        if ($item) {
            $arrs['list_id'] = explode(',', str_replace(':','',$item['list_brand_id']));
            $data['selected'] = explode(',', str_replace(':','',$item['list_brand_id']));
        }

        // Danh sách thương hiệu
        $ProductBrandTb = new ProductBrandTb();
        $data['list'] = $ProductBrandTb->listItem($arrs);
    ...
    public function relatedProductAction()
    {
        ...
        foreach (explode(',', $this->params()->fromPost('cat_related_id')) as $root_id) {
            ...
            $data['list'] = array_merge($data['list'], $ProductTb->listItem(array($key => $root_id, 'deny_id' => $data['selected'], 'columns' => array('id','name'))));
        }
    ...
    public function labelAction()
    {
        ...
        // Danh sách thương hiệu
        $ProductLabelTb = new ProductLabelTb();
        $data['list'] = $ProductLabelTb->listItem($arrs);
    ...
    {
        $this->_field['list_select_id'] = json_decode($this->_field['list_select_id'], true);
        $item = $this->_ProductCatTb->getItem(array('id' => $this->params()->fromPost('parent'), 'columns' => array('list_select_id')));
        if ($this->_params['id']) {
            $item_id = $this->_ProductCatTb->getItem(array('id' => $this->_params['id'], 'columns' => array('list_select_id')));
            $item = $item_id['list_select_id'] ? $item_id : $item;
        }
        ...
        $ProductSelectTb = new ProductSelectTb();
        $data['elements']['list_select_id'] = array(
            'label' => $select['name'],
            'name' => 'list_select_id',
            'type' => $select['define']['level'] == 2 ? 'group' : 'checkbox',
        ...
        if ($this->getRequest()->isPost()) {
            ...
            foreach (explode(',',$this->_params['id']) as $id) {
                if ($this->_params['post']['check']) {
                    $check = $this->_ProductCatTb->listItem(array('check' => $id,'count' => true));
                    echo (count($check) == 1 && $check[0]['count'] == 0) ? 0 : 1;
                } else {
                    // Xóa item || move hình ảnh qua thư mục uploads/removes
                    $item = $this->_ProductCatTb->getItem(array('id' => $id));
                    $this->_Upload->deleteImage($item, $this->_params['checkForm']['image'], $this->_field);
                    $this->_ProductCatTb->deleteItem(array('id' => $id));
                    // Thay đổi trang thái product_label_tb
                    if ($item['level'] == 1) {
                        $ProductLabelTb = new ProductLabelTb();
                        foreach (explode(',', str_replace(':','',$item['list_label_id'])) as $label_id) {
                            if($label_id) {
                                $ProductLabelTb->saveData(array('id' => $label_id, 'post' => array('status' => 0)));
                            }
                        }
                    }
                    // Xóa menu_public_tb liên quan
                    $menu = $this->_MenuPublicTb->getItem(array(
                        'active' => 'product-list-'.(($this->_params['root_id'] == 27) ? $id : $this->_params['root_id']),
                        'link' => '-'.$id.'-'.$this->_field['action']
                    ));
                    if ($menu) { $this->_MenuPublicTb->deleteItem(array('id' => $menu['id'])); }
                    // Xóa ngôn ngữ
                    if ($this->identity()->lang) {
    ...
    public function codelistAction()
    {
        ...
        $data = array(
            ...
            'list' => $this->_ProductCatTb->listItem(array('parent' => 0))
        );
    ...
    public function codefieldAction()
    {
        ...
        $data = array(
            ...
            'action' => array('2.html'=>'list2','11.html'=>'list11','12.html'=>'list12','13.html'=>'list13','14.html'=>'list14'),
            'm_brand' => $MenuCodeTb->getItem(array('id' => 13, 'columns' => array('id','name'))),
            'm_label' => $MenuCodeTb->getItem(array('id' => 15, 'columns' => array('id','name'))),
            'm_select' => $MenuCodeTb->getItem(array('id' => 17, 'columns' => array('id','name'))),
            'linkBack' => $this->url()->fromRoute('admincp',array('controller' => $this->_params['__CONTROLLER__'],'action' => $this->_params['id'] ? 'list' : 'codelist','id' => $this->_params['id'],'root_id' => $this->_params['id'] ? $this->_params['id'] : $this->_params['root_id']))
        );
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['item']['thumbnail']) {
                $this->_Upload->updateImage(array(
                    ...
                    'module' => 'single_pcat'.$this->_params['id']
                ));
            }
            if ($data['item']['multi_image']) {
                ...
                foreach ($this->_field['multi_image'] as $i => $value) {
                    $this->_Upload->updateImage(array(
                        ...
                        'module' => 'multi_pcat'.$this->_params['id'].$i
                    ));
                }
            ...
            // Lưu dữ liệu
            $id = $this->_ProductCatTb->saveData($this->_params);

            // Thêm/sửa menu_public_tb
            $arrs = array('post' => array(
            ...
            $menu = ($this->_params['id']) ? $this->_MenuPublicTb->getItem(array('active' => 'product-list-'.$id,'parent' => 0)) : '';
            if ($menu) {
                // Change link based on config action for child menu
                if ($this->_params['post']['define_item']['action'] != $this->_field['action']) {
                    foreach ($this->_MenuPublicTb->listItem(array('active' => 'product-list-'.$id)) as $value) {
                ...
                $arrs['post']['active'] = 'product-list-'.$id;
        ...
        $defines = array(
            ...
            'radio' => array('icon-link-web' => array('label' => 'Hiển thị icon link web','list' => array('none' => 'Không hiển thị', 'product-list' => 'Hiển thị'))),
