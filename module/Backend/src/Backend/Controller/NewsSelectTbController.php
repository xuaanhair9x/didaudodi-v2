...
class NewsSelectTbController extends AbstractActionController
{
    protected $_NewsSelectTb;
    ...
    public function onDispatch(\Zend\Mvc\MvcEvent $e)
    {
        ...
        $this->_NewsSelectTb = new NewsSelectTb();
        ...
        $this->_field['define']['news'] = $NewsCatTb->listItem(array('parent' => 0, 'columns' => array('id','name','define_news')));

        foreach ($this->_field['define']['news'] as $i => $value) {
            if (!in_array($this->_params['root_id'], json_decode($value['define_news'],true)['list_select_id'])) {
                unset($this->_field['define']['news'][$i]);
            }
        }
    ...
    public function listAction()
    {
        ...
        $data = array(
            ...
            'list' => $this->_renderer->cateSortNested($this->_NewsSelectTb->listItem(array('menu_code_id' => $this->_params['root_id'], 'language' => ($this->identity()->lang) ? true : false)), array('parent' => 0, 'type' => 'sort'))
        );
    ...
    public function fieldAction()
    {
        ...
        if ($this->_params['id']) {
            $data['item'] = $this->_NewsSelectTb->getItem(array('id' => $this->_params['id']));
            // Dữ liệu translate
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $items = $this->_NewsSelectTb->listItem(array(
            ...
            if ($validate->isError() == true ) {
                ...
                $data['error'] = array($this->_field['name'].': Đã tồn tại');
            } else {
                ...
                if ($this->_field['define']['level']) {
                    $level = $this->_NewsSelectTb->getItem(array('id' => $this->_params['post']['parent']));
                    $this->_params['post']['level'] = ($level) ? ($level['level'] + 1) : 1;
                }

                $this->_params['post']['list_news_cat_id'] = array();
                foreach ($this->_field['define']['news'] as $value) {
                    $this->_params['post']['list_news_cat_id'][] = $value['id'];
                }

                // Lưu dữ liệu
                $id = $this->_NewsSelectTb->saveData($this->_params, $this->_field['define']);
        ...
        if ($this->_field['define']['level']) {
            $list = $this->_renderer->cateSortNested(
                $this->_NewsSelectTb->listItem(array(
                    'menu_code_id' => $this->_params['root_id'],
    ...
    public function changeAction()
    {
        if ($this->getRequest()->isPost()) {
            foreach (explode(',',$this->_params['id']) as $id) {
                $this->_NewsSelectTb->saveData(array('id' => $id, 'post' => $this->params()->fromPost()));
            }
        }
        ...
        if ($this->getRequest()->isPost()) {
            ...
            foreach (explode(',',$this->_params['id']) as $id) {
                if ($this->_params['post']['check']) {
                    echo (count($this->_NewsSelectTb->listItem(array('parent' => $id))) == 0) ? false : true;
                } else {
                    // Xóa item || move hình ảnh qua thư mục uploads/removes
                    $item = $this->_NewsSelectTb->getItem(array('id' => $id));
                    $this->_Upload->deleteImage($item, $this->_params['checkForm']['image'], $this->_field['define']);
                    $this->_NewsSelectTb->deleteItem(array('id' => $id));
                    // Xóa ngôn ngữ
                    if ($this->identity()->lang) {
