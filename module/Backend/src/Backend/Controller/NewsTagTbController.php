...
class NewsTagTbController extends AbstractActionController
{
    ...
    public function listAction()
    {
        ...
        $data = array(
            ...
            'list' => $this->_NewsTagTb->listItem(array(
                'countNews' => true,
        ...
        if ($this->_field['define']['icon-link-web'] != 'none') {
            ...
            $tagsConfig = $NewsCatTb->listItem(array('parent' => 0, 'define_news' => array('list_tag_id', 'tags')));

            if (count($tagsConfig) == 1 || $this->_field['define']['icon-link-web'] == 'news-tag-without-category') {
    ...
    public function fieldAction()
    {
        ...
        if ($this->_params['id']) {
            ...
            $data['item'] = $this->_NewsTagTb->getItem(array('id' => $this->_params['id']));
            // Dữ liệu translate
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $item = $this->_NewsTagTb->getItem(array('name' => $this->_params['post']['name']));
            ...
            if ($validate->isError() == true || ($item && !$this->_params['id'])) {
                ...
                if ($this->_params['checkForm']['image']) {
                    $this->_Upload->uploadImage($this->_params['post'], $this->_params['checkForm']['image'], array_merge($this->_field['define'], array('listLang' => $this->identity()->langlist, 'lang' => $this->identity()->lang)), $data['item'], 'newstag'.$this->_params['root_id']);
                }
                ...
                // Lưu dữ liệu
                $id = $this->_NewsTagTb->saveData($this->_params);
    ...
    public function changeAction()
    {
        if ($this->getRequest()->isPost()) {
            foreach (explode(',',$this->_params['id']) as $id) {
                $this->_NewsTagTb->saveData(array('id' => $id, 'post' => $this->params()->fromPost()));
            }
        }
    ...
    public function deleteAction()
    {
        if ($this->getRequest()->isPost()) {
            $this->_params['post'] = $this->params()->fromPost();
            foreach (explode(',',$this->_params['id']) as $id) {
                $this->_NewsTagTb->deleteItem(array('id' => $id));
                // Xóa ngôn ngữ
                if ($this->identity()->lang) {
    ...
    public function codefieldAction()
    {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if ($data['item']['thumbnail']) {
                $this->_Upload->updateImage(array(
                    ...
                    'module' => 'single_newstag'.$this->_params['id']
                ));
            }
            if ($data['item']['multi_image']) {
                ...
                foreach ($this->_field['define']['multi_image'] as $i => $value) {
                    $this->_Upload->updateImage(array(
                        ...
                        'module' => 'multi_newstag'.$this->_params['id'].$i
                    ));
                }
        ...
        $defines = array(
            ...
            'radio' => array('icon-link-web' => array('label' => 'Hiển thị icon link web','list' => array('none' => 'Không hiển thị', 'news-tag-width-category' => 'Định dạng url có id danh mục', 'news-tag-without-category' => 'Định dạng url không có id danh mục'))),
