...
class ProductBrandTb extends AbstractTableGateway
{
    ...
    {
        $select = new Select();
        $select ->from(array('pbrand' => $this->table));
        ...
        if (!empty($params['language'])) {
            $select->join(
                ...
                new Expression('pbrand.id = l.item_id AND (l.type = "" OR l.type = "'.$this->table.'")'),
                array(
        ...
        $select->order(new Expression('CASE WHEN pbrand.sort IS NULL THEN 1 ELSE 0 END, pbrand.sort ASC, pbrand.id DESC'));
