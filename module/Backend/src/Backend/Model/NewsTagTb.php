...
class NewsTagTb extends AbstractTableGateway
{
    ...
    {
        $select = new Select();
        $select ->from(array('ntag' => $this->table));

        if ($params['countNews']) {
            $select->join(
                array('n' => 'news_tb'),
                new Expression('n.list_tag_id LIKE CONCAT("%:", ntag.id, ":%")'),
                array('list_product_id' => new Expression('GROUP_CONCAT(n.id)')),
                'left'
            );
            $select->group('ntag.id');
        }
        if (isset($params['columns'])) {
            $select->columns($params['columns']);
        }
        if (isset($params['list_news_id'])){
            $select->where->like('ntag.list_news_id', '%:'.$params['list_news_id'].':%');
        }
        if (isset($params['display'])) {
            $select->where->equalTo('ntag.display', $params['display']);
        }

        if (!empty($params['language'])) {
            $select->join(
                array('l' => 'lang_multi_tb'),
                new Expression('ntag.id = l.item_id AND (l.type = "" OR l.type = "'.$this->table.'")'),
                array(
        ...
        $select->order(new Expression('CASE WHEN ntag.sort IS NULL THEN 1 ELSE 0 END, '.(($params['orderby']) ? 'ntag.'.implode(', ntag.', $params['orderby']) : 'ntag.sort ASC, ntag.id DESC')));
    ...
    public function saveData($params = null)
    {
        ...
        if (isset($params['post']['name'])) {
            ...
            $data['title'] = isset($params['post']['title']) ? $params['post']['title'] : $params['post']['name'];
            $data['description'] = isset($params['post']['description']) ? $params['post']['description'] : $params['post']['name'];
            $ToSlug = new ToSlug();
