...
// |                                                                        |
// | Please give credit on sites that use ThumbImages and submit changes   |
 ...
 */
class ThumbImages extends AbstractHelper{
    ...
    function upload($file, $lang = 'en_GB') {
        ...
        $this->lang               = $lang;
        if ($this->lang != 'en_GB' && file_exists(dirname(__FILE__).'/lang') && file_exists(dirname(__FILE__).'/lang/ThumbImages.' . $lang . '.php')) {
            $translation = null;
            include(dirname(__FILE__).'/lang/ThumbImages.' . $lang . '.php');
        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                if (substr($file, 0, 4) == 'php:' || substr($file, 0, 5) == 'data:' || substr($file, 0, 7) == 'base64:') {
                    ...
                    $this->no_upload_check = TRUE;
                    $this->file_dst_path = ROOT_PUBLIC.'/'.UPLOAD_IMAGES.date('Y/m',explode('-',$file)[0]).'/';
                    if ($this->uploaded && !file_exists($this->file_dst_path.$file)) {
                        $this->uploaded = false;
                        $this->error = $this->translate('local_file_missing');
                    }

                    if ($this->uploaded && !is_readable($this->file_dst_path.$file)) {
                        $this->uploaded = false;
                        $this->error = $this->translate('local_file_not_readable');
                    }

                    if ($this->uploaded) {
                        $this->file_src_pathname   = $this->file_dst_path.$file;
